...
static int pfs_fill_sb(struct super_block *sb, void *data, int silent)
{
    ...
    sb->s_magic = PFS_MAGIC_NUMBER; // проверка что на диске хранится нужная файловая система
    sb->s_op = &pfs_super_ops;

    // делаем айнод передавая указатель на суперблок. mode, который задает разрешения на создаваемый файл и его тип
    ...
    /**
     * Далее для корневого каталога создается структура dentry, через которую он помещается в directory-кэш. 
     * Заметим, что суперблок имеет специальное поле, хранящее указатель на dentry корневого каталога, которое также устанавливается myfs_fill_sb
    */
...
static struct file_system_type pfs_type = {
    ...
    .name = "pfs", // используется при монтированнии 
    .mount = pfs_mount,
...
{
    int ret = register_filesystem(&pfs_type); // регистрация файловой системы 
    if (ret != 0) {
...
анализ сочетания цветов
кодогенерация на основе idef0 диаграмм (создавать idef0 по готовым функциям) (case-системы, программы написания кода по документации)
